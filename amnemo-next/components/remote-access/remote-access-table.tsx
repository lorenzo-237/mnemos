...
export function RemoteAccessTable({ machines }: RemoteAccessTableProps) {
  ...
  // Grouper par site
  const machinesBySite = machines.reduce(
    (acc, machine) => {
  ...
  const sortedMachinesBySite = Object.values(machinesBySite).sort((a, b) =>
    a.site.name.localeCompare(b.site.name),
  );

  return (
    <div className="space-y-6">
      {sortedMachinesBySite.map(({ site, machines: siteMachines }) => (
                  ...
                  <div className="flex items-center gap-4 flex-1">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-xs text-muted-foreground mt-0.5">
                        {machine._count.installations} logiciel
                        {machine._count.installations > 1 ? "s" : ""}
                      </div>
                    </div>
                    ...
                    {machine.teamviewerId ? (
                      <div className="flex items-center gap-2">
                        ...
                        <Button
                          ...
                          size="sm"
                          onClick={() =>
                            copyTeamViewerId(machine.teamviewerId!)
                          }
                        >
                          <HugeiconsIcon
                            icon={Copy01Icon}
