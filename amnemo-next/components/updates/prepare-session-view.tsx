              ...
              .map((task) => (
                <Card key={task.id}>
                  ...
                  <CardContent className="space-y-3">
                    ...
                    {(() => {
                      const targetSoftwareId = softwareReplacements[task.id]?.softwareId;
                      if (!targetSoftwareId) return null;
                      const machinesWithout = selectedMachines.filter(machineId => {
                        const machine = availableMachines.find(m => m.id === machineId);
                        return !machine?.installations?.some((inst: any) => inst.software.id === targetSoftwareId);
                      });
                      if (machinesWithout.length === 0) return null;
                      const targetSoftware = softwares.find(s => s.id === targetSoftwareId);
                      return (
                        <div className="bg-amber-50 dark:bg-amber-950 border border-amber-200 dark:border-amber-800 rounded-md px-3 py-2 mt-1">
                          <p className="text-xs text-amber-700 dark:text-amber-300 font-medium">
                            ⚠ Le logiciel {targetSoftware?.name || '?'} n'est pas installé sur :
                          </p>
                          <p className="text-xs text-amber-600 dark:text-amber-400 mt-0.5">
                            {machinesWithout.map(machineId => {
                              const machine = availableMachines.find(m => m.id === machineId);
                              return machine ? `${machine.name} (${machine.site.name})` : '';
                            }).join(', ')}
                          </p>
                          <p className="text-xs text-amber-600 dark:text-amber-400 mt-0.5">
                            Il sera ajouté lors de la validation de cette tâche.
                          </p>
                        </div>
                      );
                    })()}
